...
with open("qwen2.5vl32b.txt", "r", encoding="utf-8") as f:
    vlm = f.read().splitlines()
vlmText = []
for line in vlm:
    vlmText.extend([word.upper() for word in line.split()])
...
vlmCorrect = [w for w in vlmText if w.upper() in originalText]
...
print(f"VLM qwen2.5-vl:32b OCR-Uebereinstimmungen: {vlmCorrect}\n\n\n")
...
print(f"Erkennte Woerter von VLM qwen2.5-vl:32b: {len(vlmCorrect)}/{oriNumber}")
